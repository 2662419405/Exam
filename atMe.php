<?php
    ...
    $tiao=$_GET['friend_id'];//跳到对方的homepage页面
    
	if(empty($_GET['friend_id'])){
		echo "<head><meta charset='utf-8'></head>";
        echo "<script> alert('你无权查看别人的隐私');window.location.href='homepage.php';</script>";
	}else{
		if($_GET['friend_id']!=$user_id){
			echo "<head><meta charset='utf-8'></head>";
        	echo "<script> alert('你无权查看别人的隐私');window.location.href='homepage.php?friend_id=$tiao';</script>";
		}
	}
	...
	$sql="select * from at where user_id = :user_id order by id desc limit ".(($pageNum-1)*$pageSize).",".$pageSize;
	$at_lists = $db->query($sql,array('user_id'=>$user_id));
	$total = $db->single("select count(*) from at where user_id = :user_id",array('user_id'=>$user_id));
	$yeshu=ceil($total/$pageSize);
    
    if(isset($at_lists)){
        foreach($at_lists as $vo){
            $post   = $db->row('select * from post where id = :id',array('id'=>$vo['friend_id']));

<?php
	...
	if(($name=='全部')&&($xibie=='全部')&&($xuanxiang==0)){
		$sql="select * from exam_problem where exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}
	else if(($name=='全部')&&($xibie=='全部')){
		$sql="select * from exam_problem where pro_type =:xuanxiang and exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('xuanxiang'=>$xuanxiang,'shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}
	else if(($xibie=='全部')&&($xuanxiang==0)){
		$sql="select * from exam_problem where exam_name =:name and exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('name'=>$name,'shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}
	else if(($name=='全部')&&($xuanxiang==0)){
		$sql="select * from exam_problem where chinese =:xibie and exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('xibie'=>$xibie,'shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}elseif($name=='全部'){
		$sql="select * from exam_problem where chinese =:xibie and pro_type =:type and exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('xibie'=>$xibie,'type'=>$xuanxiang,'shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}elseif($xuanxiang==0){
		$sql="select * from exam_problem where chinese =:xibie and exam_name =:name and exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('xibie'=>$xibie,'name'=>$name,'shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}elseif($xibie=='全部'){
		$sql="select * from exam_problem where pro_type =:type and exam_name =:name and exam_addtime between :shijian and :jiezhi";
		$re=$db->query($sql,array('type'=>$xuanxiang,'name'=>$name,'shijian'=>$shijian,'jiezhi'=>$jiezhi));
	}else{
		$sql="select * from exam_problem where chinese =:xibie and pro_type =:type and exam_name =:name and exam_addtime between :shijian and :jiezhi";
		...
		<div class="main_content">
			...
			<div class="main_content_index">
				<form action="success.php" method="post">
					<input type="hidden" name="biaoji" value="1"/>

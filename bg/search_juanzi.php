<?php
	...
	$sql="select * from exam_zhuanye where name like :keyword group by name order by exam_addtime desc limit ".(($pageNum-1)*$pageSize).",".$pageSize;
	$chaxun = $db->query($sql,array('keyword'=>"%$keyword%"));
	$t=$db->query("select * from exam_zhuanye where name like :keyword group by name",array('keyword'=>"%$keyword%"));
	$toutal=sizeof($t);
            		...
            		<div class="col">
            			...
            			<div class="box_content">
            				...
            				<div class="seach_shiti">
            					<form action="search_juanzi.php" method="get">
            						<input type="text" placeholder="请输入卷子名称" id="sou" name="keyword"  onKeyPress="if(event.keyCode==13) {document.getElementById('dianji').click();}"/>
            			...
            			<form action="quanshan_juanzi.php" method="post">
	            				...
	            				<tbody>
	            					<?php
	            						foreach($chaxun as $vo){
	            							$shiti_id=$vo['id'];
	            							$shiti_zhuanye=$vo['name'];
	            							$shiti_zuozhe=$vo['chinese'];
	            							$shiti_z=$vo['exam_name'];
	            							$shiti_shijian=$vo['exam_addtime'];
	            							?>
	            						<tr>
	            							<td>
	            								<input class="shanchu" type="checkbox" name="ids[]"  value="<?php echo $shiti_zhuanye?>"/>
	            							</td>
	            							<td>
	            								<?php echo $shiti_id?>
	            							</td>
	            							...
	            							<td>
	            								<a href="bianji_juanzi.php?shijuan_name=<?php echo $shiti_zhuanye?>">编辑</a>
	            								<a href="delete_juanzi.php?shijuan_name=<?php echo $shiti_zhuanye?>">删除</a>
	            							</td>
	            						</tr>
	            					<?}?>
	            				</tbody>
            			...
            			<?php include_once("bg_view/common/juanzi_page.php"); ?>
            		</div>
            	</div>
            </div>
		</div>
...
<script type="text/javascript">
	...
	add.onclick=function(){
		window.location.href='add_juanzi.php';
	}
</script>
</html>